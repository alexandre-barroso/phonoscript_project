...
package_name="PhonoScript-macos-$architecture"
...
fixture_smoke="$(mktemp -d "$platform_root/fixture-smoke.XXXXXX")"
trap 'rm -rf "$fixture_smoke"' EXIT
while IFS= read -r -d '' fixture; do
  emitted="$fixture_smoke/$(basename "${fixture%.ottab}").phont"
...
# This local ad-hoc signature validates bundle-independent CLI integrity. A
# public macOS release still requires the owner's Developer ID and notarization.
codesign --force --sign - "$package_root/bin/phonoscript"
codesign --verify --strict --verbose=2 "$package_root/bin/phonoscript"
xattr -cr "$package_root"

COPYFILE_DISABLE=1 tar -C "$platform_root" -czf "$archive_path" "$package_name"

archive_smoke="$fixture_smoke/archive"
mkdir -p "$archive_smoke"
tar -C "$archive_smoke" -xzf "$archive_path"
extracted="$archive_smoke/$package_name"
codesign --verify --strict --verbose=2 "$extracted/bin/phonoscript"
...
install_prefix="$fixture_smoke/prefix"
